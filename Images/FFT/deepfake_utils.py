...
def compute_metrics(all_labels, all_probs):
    ...
    return dict(acc=acc, auc=auc, ap=ap, precision=precision, recall=recall, f1=f1)
...
def bce_with_label_smoothing(pred, target, epsilon=0.1, pos_weight=None):
    ...
    if pos_weight is not None:
        ...
        loss = torch.nn.functional.binary_cross_entropy(pred, smooth_target, weight=weight)
    else:
        loss = torch.nn.functional.binary_cross_entropy(pred, smooth_target)
    return loss